...
#------------------------GUI start-------------------------#
   ...
   inst= "Proper detalis required to link lyrics with iTunes\n\nFILL CAREFULLY"
...
#-----------------------from CLI------------------------------------#
    ...
    for info in playlist:
        ...
        '''If not on macOS , next following lines wont work
        toggle to comments or remove '''

        song = info[1]      #to search in iTunes as a parttern
        artist = info[0]
        success = lyTag.addToITunes(song,artist,lyrics)
        #print(success)
        if success == False:
            messagebox.showwarning("iTunes link failed","Song:"+info[3]+"\nCheck artist name in iTunes")
            messagebox.showwarning("Check iTunes","you entered:\nArtist:"+info[0]+"  Song:"+info[1])
